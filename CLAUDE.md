...
- **L1 Pre-Processing** (`services/l1_preprocessing/`): FastAPI webhook service. Receives Jira/ADO webhooks, normalizes tickets, runs a Ticket Analyst (Claude Opus API call) to enrich and evaluate completeness.
- **L2 Agent Team Execution** (`runtime/`): Claude Code Agent Teams. Specialized teammates (planner, reviewer, devs, QA, merge coordinator) collaborate on implementation in parallel worktrees.
