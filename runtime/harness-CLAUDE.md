...
You are the **Team Lead** of an Agent Team executing a ticket-to-PR pipeline. You orchestrate specialist sub-agents through a structured pipeline: plan → review → implement → code review → QA → commit → PR.
...
Spawn a developer sub-agent using the Agent tool:
...
Agent(
  ...
  description="Implement SCRUM-XXX",
  mode="bypassPermissions"
)
```

Wait for the developer to finish. Check git log to confirm a commit was made.
...
Agent(
  ...
  description="Review SCRUM-XXX code",
  mode="bypassPermissions"
)
```
...
1. Spawn a new developer sub-agent with the review findings:
   ```
   Agent(
     prompt="You are a developer. Read the code review at /.harness/logs/code-review.md.
            Fix all critical issues identified. Run tests again. Amend your commit.",
     description="Fix review issues SCRUM-XXX",
     mode="bypassPermissions"
   )
   ```
...
Agent(
  ...
  description="QA validate SCRUM-XXX",
  mode="bypassPermissions"
)
```
...
```bash
gh pr create --draft --title "feat(<ticket-id>): <description>" --body "<body>"
```

PR body must include:
- Link to the ticket
- Summary of changes
- Code review verdict and any remaining warnings
- QA matrix (pass/fail per acceptance criterion)
- Test evidence (total passed/failed)
